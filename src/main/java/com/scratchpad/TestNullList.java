...
public class TestNullList {
     ...
     * @return
     */
    public static ERDSAggregate transformToEntity(
            ERDSAggregate erdsAggregateEntity, String tildeSeperatedString,
            Map<Long, String> empMap) {
        StringBuilder ecmName = new StringBuilder();
        if (StringUtils.isNotBlank(tildeSeperatedString)) {
            for (String ecmCd : tildeSeperatedString.split("\\s*~\\s*")) {
...
class ERDSAggregate {
    ...
    /**
     * @param ecmEmpName the ecmEmpName to set
     */
